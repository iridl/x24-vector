...
def onset_index(
    data, data_flag, wlength,      wthresh,       flagged_thresh, fwlength, false_search, coord_index
):
    ...
    not followed by a false_window of size fwlength of unflagged data, for the following false_search indices
    ...
    window = (
        data.rolling(**{coord_index: wlength}).sum() > wthresh
    ) & (
    ...
    unflagged = ~flagged
    false_window = (
        unflagged.rolling(**{coord_index: fwlength}).sum() == fwlength
    )

#Note that rolling assigns to the last position of the window
    false_window_ahead = (
        false_window.rolling(**{coord_index: false_search}).sum().shift(**{coord_index: false_search*-1}) != 0
    )

    flagged_within_onset = (
        flagged
    ) & (
        (window & ~false_window_ahead).rolling(**{coord_index: wlength}).sum().shift(**{coord_index: 1-wlength}) != 0
    )
...
#Note it doesn't matter to use idxmax or idxmin, this finds the first max/min thus the first onset date since we have only 1s and nans
...
def run_test():
  ...
  print(
    onset_index(rr_mrg.precip, 1, 3, 20, 1, 7, 21, "T")
  )
